        ...
        function print_html_result_row(obj, prefix, i)
            ...
            for j=1:numel(obj.config_cache)
                ...
                if isempty(strfind(info.status,'Inaccurate'))
                    if (info.solveTime < 100)
                        fprintf(obj.stream_fid, ...
                            ['%s  <td><span class="mm_runtime" title="runtime in milliseconds">%.1f</span> ' ...
                            '(<span class="mm_iterations" title="iterations">%d</span>)</td>\n'], ...
                            prefix, ...
                            info.solveTime, info.iter);
                    else
                        fprintf(obj.stream_fid, ...
                            ['%s  <td><span class="mm_runtime" title="runtime in milliseconds">%d</span> ' ...
                            '(<span class="mm_iterations" title="iterations">%d</span>)</td>\n'], ...
                            prefix, ...
                            round(info.solveTime), info.iter);
                    end
                else
                    fprintf(obj.stream_fid, ...
                        ['%s  <td><span class="mm_runtime mm_runtime_failed" title="failed, time=%.2f min" style="color:red;">%s</span> ' ...
                        '(<span class="mm_iterations mm_iterations_failed" title="iterations">%d</span>)</td>\n'], ...
